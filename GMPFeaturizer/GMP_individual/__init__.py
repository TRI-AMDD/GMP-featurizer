...
from ._libgmpind import ffi, lib


class GMPInd(BaseFeature):
    """
    ...
    def _load_psp_files(self):
        ...
        # ngaussian_list = list()
        ngaussian_list = np.zeros(120, dtype=np.intc, order="C")
        ...
        for i, _ in enumerate(element_index_to_order_list):
            element_index_to_order_list[i] = i
    ...
    def calculate_features(
        ...
        size_info = np.array([atom_num, cal_num, self.params_set["total_num"]])

        if self.solid_harmonic:
            if calc_derivatives is False and calc_occ_derivatives is False:
                # print(self.params_set["num"])
                # print(self.params_set["total_num"])
                x = np.zeros(
                    [cal_num, self.params_set["total_num"]], dtype=np.float64, order="C"
                )
                ...
                if self.custom_cutoff == 4:
                    ...
                    errno = lib.calculate_solid_gmp_elemental_sigma_gaussian_cutoff_noderiv_opt2(
                        cell_p,
                        cart_p,
                        occupancies_p,
                        ref_cart_p,
